<template>
  <div class="space-y-6 bg-white text-black dark:bg-black dark:text-white p-4 rounded">
    <h1 class="text-3xl font-bold text-gray-800 mb-8 dark:text-white">설정</h1>
          ...
          >
            ...
            <label class="relative inline-flex items-center cursor-pointer ml-4">
              <input
                type="checkbox"
                class="sr-only peer"
                v-model="settings[notification.key]"
              />
              <!-- tailwind가 주는 토글🔽 -->
              <div
...
const backupData = () => {
  alert("데이터 백업이 시작 되었습니다.");
};
const restoreData = () => {
  if (confirm("데이터 복원 시 현재 데이터가 덮어씌워집니다. \n계속 진행하시겠습니까?")) {
